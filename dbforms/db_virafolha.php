...
include("./classes/db_rhpessoalmovcontabancaria_classe.php");
...
require_once("./libs/db_app.utils.php");
  ...
  if(isset($desprocess) && $desprocess == "true"){
    ...
    $sWhereRhEmpenhoFolha = "rh72_anousu = $dataii_ano and rh72_mesusu = $dataii_mes";
    $sSqlRhEmpenhoFolha   = $oDaoRhEmpenhoFolha->sql_query_file(null, "rh72_sequencial", null, $sWhereRhEmpenhoFolha);
    $rsRhEmpenhoFolha     = db_query($sSqlRhEmpenhoFolha);

    if (pg_num_rows($rsRhEmpenhoFolha) > 0) {
    ...
    if (isset($DB_COMPLEMENTAR) && $DB_COMPLEMENTAR) {
      ...
      try {
        ...
        if ($oDaoFolhaPagamento->erro_status == "0"){
          throw new DBException( _M(FolhaPagamento::MENSAGENS . "erro_excluir"));
        }
      } catch (Exception $oException) {
