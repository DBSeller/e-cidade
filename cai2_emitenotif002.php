...
if ( $tiporel == 1 or $tiporel == 11 ) {
    ...
    for ($doc = 0; $doc < pg_numrows($resultdocparagr); $doc++) {
      ...
      if (strtoupper($db02_descr) == "TOTALPORANO" and $somenteparc == false) {
        ...
        if (!isset($passou_por_aqui)||$passou_por_aqui==false){
          ...
          if (pg_numrows($resulttipoparc) == 0 || $passar == false || ($k40_todasmarc == 't'?pg_numrows($resultcompara1) <> pg_numrows($resultcompara2):false)) {
            $desconto = 0;
          } else {
        ...
        /**
         * @todo mover para query unica
        * $sqlinstit = "select nomeinst, ender, numero, munic, cgc, email, telef, uf, logo, to_char(tx_banc,'9.99') as tx_banc, numbanco
        ...
        */
        ...
        $NumBanco     = $numbanco;  //pg_result($DadosInstit,0,"numbanco");
        $taxabancaria = $tx_banc;   //pg_result($DadosInstit,0,"tx_banc");
        $src          = $logo;      //pg_result($DadosInstit,0,'logo');
        $db_nomeinst  = $nomeinst;  //pg_result($DadosInstit,0,'nomeinst');
        $db_ender     = $ender;     //pg_result($DadosInstit,0,'ender');
        $db_munic     = $munic;     //pg_result($DadosInstit,0,'munic');
        $db_cgc       = $cgc;       //pg_result($DadosInstit,0,'cgc');
        $db_uf        = $uf;        //pg_result($DadosInstit,0,'uf');
        $db_telef     = $telef;     //pg_result($DadosInstit,0,'telef');
        $db_email     = $email;     //pg_result($DadosInstit,0,'email');
  ...
  function EnderecoNotif($tratamento, $matric, $z01_nome ) {
    ...
    global $z01_ender,$z01_bairro, $z01_numero, $z01_compl, $z01_munic, $z01_uf, $z01_cep, $z01_cxpostal, $z01_destinatario, $sCepCxPostal, $sMunicipio, $lValidaOrdemEnderecoEntrega;
