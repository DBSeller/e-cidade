  ...
  public function aditar($aItens, $iTipoAditamento, $dtVigenciaInicial, $dtVigenciaFinal, $sNumeroAditamento = null) {
    ...
    switch ($iTipoAditamento) {
      ...
      case self::TIPO_ADITAMENTO_QUANTIDADE_VALOR:
        ...
        $oValoresContrato = $this->getValoresItens();
    ...
    foreach ($aItens as $oItem) {
      ...
      /**
       * Devido ao número de erros ocorrentes por causa da origem vir com dados ora com ponto ora com virgula,
       * valido aqui, caso a virgula seja encontrada, substituo os dados para que seja salvo corretamente no
       * banco de dados.
       */
      if (strpos($oItem->valorunitario, ',')) {

        $aValorItem           = explode(",", $oItem->valorunitario);
        $aValorItem[0]        = str_replace('.', '', $aValorItem[0]);
        $oItem->valorunitario = implode('.', $aValorItem);
      }
      ...
      $oNovoItem->setPeriodosExecucao($this->iCodigoAcordo, $this->lPeriodoComercial, $iTipoAditamento);
