...
class DadosCensoDocente extends DadosCenso {
  ...
  protected function getDados() {
    ...
    $sWhereCgm .= " and (ed01_funcaoatividade in (2,3,4) or (ed01_funcaoatividade = 1 and ed01_c_regencia = 'S'))";
  ...
  public function getEnderecoDocumento() {
    ...
    $sWhereCgm  = " ed20_i_codigo = {$this->getCodigoDocente()} ";
    $sWhereCgm .= "  and ed01_funcaoatividade in (1,2,3,4) ";
    
    $sSqlDadosRecurso  = $oDaoRecursoHumano->sql_query_censo(null, $sCampos, null, $sWhereCgm);
    $rsRecurso         = $oDaoRecursoHumano->sql_record($sSqlDadosRecurso);
    if ($oDaoRecursoHumano->numrows == 0) {
      throw new Exception('Docente sem documentos lançados.');
    }
  ...
  public function getDadosFormacao() {
    ...
    $sCampos            .= " case ";
    $sCampos            .= "   when ed27_i_censoinstsuperior = 9999999 ";
    $sCampos            .= "     then 2                                 ";
    $sCampos            .= "   else ed257_i_tipo                       ";
    $sCampos            .= " end as tipo_instituicao                   ";
  ...
  protected function getDadosOutrosCursos() {
     ...
     */
    $oDaoRecHumanoDadosCenso = new cl_rechumanodadoscenso();
    ...
    if (!$rsPerguntas) {
      throw new Exception("Erro ao consultar outros dados de cursos dos profissionais");
    }
  ...
  public function getDadosDocencia() {
    ...
    $sWhere  .= " and (ed75_i_saidaescola is null OR ed75_i_saidaescola  >= '{$this->dtBaseCenso}') ";
    $sWhere  .= " and ed75_i_escola = {$this->getCodigoEscola()}";
    ...
    $sCampos .= "                            case when ed342_censoetapa is not null then ed342_censoetapa else ed57_i_censoetapa end as censo_etapa,";
    $sCampos .= "                            1 as funcao,";
    ...
    foreach ( $aDocencia as $oDocente) {
      ...
      for ($iDisciplina = 0; $iDisciplina < 13; $iDisciplina++) {
        ...
        if ( !in_array($oDocente->censo_etapa, $aEtapasCensoTurma) && ($oDocente->ed57_i_tipoturma != 4 && $oDocente->ed57_i_tipoturma != 5) ) {
  ...
  public function getDadosDocenciaTurmaAEE() {
    ...
    $sWhere             = "     ed346_rechumano    = {$this->getCodigoDocente()}";
    $sWhere            .= " and ed52_i_ano     = {$this->getAnoCenso()}";
  ...
  protected function getOutrasFuncoesExercidas() {
    ...
    $rsDadosDocente          = $oDaoOutrosProfissionais->sql_record($sSqlTurmasRecursoHumano);
    return db_utils::getCollectionByRecord($rsDadosDocente);
  }

  public function setDataCenso($sDataCenso) {
    $this->dtBaseCenso = $sDataCenso;
  }
}
