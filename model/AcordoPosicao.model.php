...
class AcordoPosicao {
 ...
 /**
  ...
  /**
      ...
      Alterar o método getItensAditar, fazendo com que retorne uma propriedade chamada "oSaldo" que o saldo criado no método "processarSaldo()"
   *  private static function processarSaldoItem

      deve receber um array de periodos onde irá processar os saldos retornando um objeto stdclass com as seguintes propriedades

      quantidade_total = soma da ->quantidade
      quantidade_executada = soma da  ->executado
      quantidade_restante = soma da  ->saldo
  ...
  public function getItensAditar ($lRenovacao = false) {
    ...
    foreach ($this->getItens() as $oItemPosicao) {
      ...
      $oItem->oSaldo = self::processarSaldoItem( $aPeriodos );
      if ($oItemPosicao->getTipocontrole() == AcordoItem::CONTROLE_QUANTIDADE || $oItemPosicao->getTipocontrole() == AcordoItem::CONTROLE_VALOR) {
        $oItem->oSaldo->quantidade_restante = $oItemPosicao->getQuantidade();
      }
  ...
  /**
   * metodo que ira criar a propriedade oSaldo, para o item a aditar
   *   contera valores sobre execuções do item
   *
   *@param  array aPeriodosItem : arrey de periodos do item
   *@return object stdClass
   *
   */
  private static function processarSaldoItem( $aPeriodosItem ){

    $oSaldo = new stdClass();
    $oSaldo->quantidade_total     = 0; //quantidade_total     = soma da ->quantidade
    $oSaldo->quantidade_executada = 0; //quantidade_executada = soma da  ->executado
    $oSaldo->quantidade_restante  = 0; //quantidade_restante  = soma da  ->saldo

    foreach ($aPeriodosItem as $iIndice => $oDados ) {

      $oSaldo->quantidade_total     += $oDados->quantidade;
      $oSaldo->quantidade_executada += $oDados->executado;
      $oSaldo->quantidade_restante  += $oDados->saldo;
    }
    return $oSaldo;
  }
