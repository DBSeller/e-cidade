...
class AjustePrevidencia {
 ...
 /**
  * Função que soma as bases de previdencia de salario e férias, 
  * a fim de reajustar o calculo de previdencia para Rescisão.
  * @param integer $iMatricula Matricula do servidor que será ajustado
  * @static
  * @access public
  * @return void
  */
 static function ajustarRescisao($iMatricula) {

    $aRubricas = array(1=>'R903', 2=>'R906', 3=>'R909', 4=>'R912');

    /**
     * Instanciamos os dados de calculo de rescisão do servidor. 
     **/
    $oServidor                   = ServidorRepository::getInstanciaByCodigo($iMatricula, DBPessoal::getAnoFolha(), DBPessoal::getMesFolha());
    $sRubricaDescontoPrevidencia = $aRubricas[$oServidor->getTabelaPrevidencia()];
    $oCalculo                    = $oServidor->getCalculoFinanceiro(CalculoFolha::CALCULO_RESCISAO);
    /**
     * Retorna o evento financeiro da rubrica R985(base de previdencia do salário) 
     */
    $aEventos                    = $oCalculo->getEventosFinanceiros(null,'R985');
    
    if ( count($aEventos) ) {
      $nValorBaseSalario         = $aEventos[0]->getValor();
    }
    /**
     * Retorna o evento financeiro da rubrica R987(base de previdencia de férias) 
     */
    LogCalculoFolha::write("Valor da Base de Salario...: $nValorBaseSalario");
    $aEventos                   = $oCalculo->getEventosFinanceiros(null,'R987');

    if ( count($aEventos) ) {
      $nValorBaseFerias         = $aEventos[0]->getValor();
    }

    LogCalculoFolha::write("Valor da Base de Férias....: $nValorBaseFerias");
    
    /**
     * Instanciamos a tabela de calculo de rescisão e somamos o valor das bases de ferias e salário.
     * E usamos o valor para atualizar a rubrica de previdencia de salário.
     */
    $oDaoGerfRes             = new cl_gerfres;
    $oDaoGerfRes->r20_anousu = DBPessoal::getAnoFolha();
    $oDaoGerfRes->r20_mesusu = DBPessoal::getMesFolha();
    $oDaoGerfRes->r20_rubric = 'R985';
    $oDaoGerfRes->r20_regist = $iMatricula;
    $oDaoGerfRes->r20_valor  = $nValorBaseSalario + $nValorBaseFerias;
    /**
     * Exclui as rubricas referentes a previdencia de férias do cálculo.
     */
    $oDaoGerfRes->excluir(DBPessoal::getAnoFolha(), DBPessoal::getMesFolha(), $iMatricula, 'R987');
    $oDaoGerfRes->excluir(DBPessoal::getAnoFolha(), DBPessoal::getMesFolha(), $iMatricula, $sRubricaDescontoPrevidencia);
    /**
     * Altera a rubrica referente a previdencia de salário, somando as bases de salario e ferias.
     */
    $oDaoGerfRes->alterar(DBPessoal::getAnoFolha(), DBPessoal::getMesFolha(), $iMatricula, 'R985');
  }
 ...
 static function gravarModificacoes (){
    ...
    if( $pessoal[$Ipessoal]["r01_tbprev"] > 0){
      ...
      if( $opcao_geral == 1 ){
        ...
        $sigla1 = "r20_";
        /**
         * Chamamos a função para realizar os ajustes de previdência
         */
        AjustePrevidencia::ajustarRescisao($pessoal[$Ipessoal]['r01_regist']);
        ...
        for($nosx=1;$nosx <4;$nosx++){
          ...
          if( $matrizr[$nosx] != ""){
            ...
            if( db_selectmax( "transacao", "select * from previden ".bb_condicaosubpes( "r60_" ).$condicaoaux )){
              LogCalculoFolha::write("Alterando os valores da tabela previdencia.");
    ...
    if (db_selectmax("previden_", "select * from previden ".bb_condicaosubpes("r60_") ) ) {
      ...
      for ($Ipreviden=0; $Ipreviden<count($previdencia_); $Ipreviden++) {
        ...
        if (db_selectmax("previden_", $sql )) {
          ...
          LogCalculoFolha::write('VAMOSSSSSSSSSS TIMEEEE: '.$rubrica_desconto); 
          }
          ...
          for($nro=0;$nro<count($mat_r60_numcgm);$nro++){
            if($opcao_geral != 3 ) {
              ...
              if (db_selectmax($arquivo, "select * from ".$arquivo." ".bb_condicaosubpes($sigla_ajuste ).$condicaoaux )) {
                ...
                LogCalculoFolha::write("Difierença no Ajuste....: {$ajusta_dif}");
            ...
            AjusteIRRF::gravarModificacoes($numcgm,$mat_r60_regist[$nro],$mat_r60_tpvinc[$nro]);

            // echo "<BR> 2 AjusteIRRF::gravarModificacoes($numcgm,".$mat_r60_regist[$nro].",".$mat_r60_tpvinc[$nro].");";
            ...
            if ($db21_codcli == "999999999" && $db21_codcli == "18" ) {
              $salario_familia = 0;
            }
