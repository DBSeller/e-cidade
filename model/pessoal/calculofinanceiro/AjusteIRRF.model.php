...
abstract class AjusteIRRF {
 ...
 /**
  * Responsável por juntar os valore s de salário e férias para wque o ajuste de irrf faça a proporcionalidade
  * de forma correta.
  *
  * @param interger $iMatricula
  * @static
  * @access public
  * @return void
  */
 static function ajustarRescisao($iMatricula) {
     ...
     **/
    $oCalculo                = ServidorRepository::getInstanciaByCodigo($iMatricula, DBPessoal::getAnoFolha(), DBPessoal::getMesFolha())->getCalculoFinanceiro(CalculoFolha::CALCULO_RESCISAO);
    /**
     * Retorna o evento financeiro da rubrica R985(base de previdencia do salário) 
     */
    $aEventos                = $oCalculo->getEventosFinanceiros(null,'R981');
    $nValorBaseSalario       = 0;
    $nValorBaseFerias        = 0;
    ...
    /**
     * Retorna o evento financeiro da rubrica R985(base de previdencia do férias) 
     */
    $aEventos                = $oCalculo->getEventosFinanceiros(null,'R983');
     ...
     */
    $oDaoGerfRes->excluir(DBPessoal::getAnoFolha(), DBPessoal::getMesFolha(), $iMatricula, 'R983');
    $oDaoGerfRes->excluir(DBPessoal::getAnoFolha(), DBPessoal::getMesFolha(), $iMatricula, 'R915');
    /**
     * Altera a rubrica referente a previdencia de salário, somando as bases de salario e ferias.
     */
    $oDaoGerfRes->alterar(DBPessoal::getAnoFolha(), DBPessoal::getMesFolha(), $iMatricula, 'R981');
  }
  /**
  ...
  static public function gravarModificacoes($numcgm, $registrop, $r01_tpvinc) {
    ...
    if ( $opcao_geral == PONTO_SALARIO ) {
      ...
      AjusteIRRF::ajustarRescisao($registrop);
    ...
    if (count($pessoal_) > 1) {
      ...
      for ($Itransacao=0; $Itransacao< count($transacao); $Itransacao++) {
        ...
        if ( in_array($sRubricaTransacao, $matrizr) ) { 
          ...
          if ( $opcao_geral != PONTO_RESCISAO ) {
            ...
            if ( $sRubricaDesconto == "R981" && $opcao_geral != PONTO_SALARIO ) {
      ...
      for ($Itransacao=0; $Itransacao<count($transacao); $Itransacao++) {
        ...
        if ( $opcao_geral != PONTO_RESCISAO ) {
          ...
          if ( $sRubricaTransacao == "R981" && $opcao_geral != PONTO_SALARIO ) {
            LogCalculoFolha::write("Nao calcula R981 fora do Ponto de Salário");
            continue; // R981 BASE I.R.F. BASE
      ...
      while ($Iajuste < $tot_ajusteir) {
        ...
        if ( $sequencia  == 3 ) { //Férias
          ...
          $sqlfer = " select sum(r31_valor) as  soma_desc_previdencia
            ...
            and r31_rubric in ('R903','R906','R909','R912') 
            and r31_tpp = 'F' 
            and rh01_numcgm = $numcgm ";
...
if (db_selectmax("previden", $sqlfer )) {
  $soma_desc_previdencia = $previden[0]["soma_desc_previdencia"];
          ...
          $condicaoaux  = " and r60_rubric = '{$aRubricasPrevidencia[$rubrica_base]}' and r60_numcgm = ".db_sqlformat($numcgm );
          global $previden;
          LogCalculoFolha::write("select * from previden ".bb_condicaosubpes("r60_" ).$condicaoaux );
