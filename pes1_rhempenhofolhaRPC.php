...
try {
  ...
  switch ($oPost->sMethod) {
    ...
    case 'consultaComplementaresFechadas':
      ...
      $sSqlPontoComplementar = $clgerfcom->sql_query_complementar_fechadas($oPost->iAnoFolha, $oPost->iMesFolha );
    ...
    case 'consultarEmpenhos':
      ...
      if (isset($DB_COMPLEMENTAR)){
        ...
        $iMesFolha    = $oPost->iMesFolha;
        $iAnoFolha    = $oPost->iAnoFolha;
        $sSigla       = $oPost->sSigla;
        $oCompetencia = new DBCompetencia($iAnoFolha,$iMesFolha);
        
        if( $sSigla == "r48" && FolhaPagamentoComplementar::hasFolhaAberta($oCompetencia) ) {
          throw new BusinessException("Não existe folha de pagamento complementar fechada para esta competência.");
        } 

        if( $sSigla == "r14" && FolhaPagamentoSalario::hasFolhaAberta($oCompetencia) ) {
          throw new BusinessException("Não existe folha de pagamento de salário fechada para esta competência.");
        }
