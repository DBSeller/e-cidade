...
if (! isset($HTTP_POST_VARS ["reduz"])) {
  ...
  $sqlconta  = "select k13_reduz as c01_reduz,                                             ";
  $sqlconta .= "       k13_descr as c01_descr                                              ";
  $sqlconta .= "  from cfautent                                                            ";
  $sqlconta .= "       inner join cfautentconta on k16_id     = k11_id                     ";
  $sqlconta .= "       inner join saltes        on k13_conta  = k16_conta                  ";
  $sqlconta .= "       inner join conplanoreduz on c61_reduz  = k13_conta                  ";
  $sqlconta .= "                               and c61_anousu = ".db_getsession("DB_anousu"); 
  $sqlconta .= "       inner join conplano      on c60_codcon = c61_codcon                 ";
  $sqlconta .= "                               and c60_anousu = c61_anousu                 ";
  $sqlconta .= " where k11_ipterm = '".db_getsession("DB_ip")."'                           ";
  $sqlconta .= "   and k11_instit = ".db_getsession("DB_instit");                             
  ...
  <table width="99%">
    <tr>
      ...
      <td align="left" valign="middle">
        <select onChange="js_atualizaconta(this.name)" name="reduz" id="reduz">
        <?
        ...
        <select onChange="js_atualizaconta(this.name)" name="descr" id="descr">
        <?
          for($i = 0; $i < pg_numrows($result_conta); $i ++) {
            db_fieldsmemory($result_conta, $i);
